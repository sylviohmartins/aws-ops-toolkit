...
| DynamoDB tipado/document API | `dynamodb-enhanced` | Schemas conhecidos, `DynamoTableGateway<T>` e modo Document | Low-level SDK | **Sim, atual** | SDK oficial gerenciado pelo AWS BOM; typed e schemaless coexistem |
| Ledger de escrita | `org.xerial:sqlite-jdbc` | Journal transacional, tasks/effects/audit | JSON apenas na foundation | **Sim, atual** | Versão centralizada; WAL/FULL e locking são exercitados localmente; filesystem corporativo ainda requer homologação |
| CSV reutilizável | `org.apache.commons:commons-csv` | Escaping/quoting robustos no `CsvReportWriter` streaming | Writer artesanal/Jackson CSV | **Sim, atual** | Pequena dependência específica; evita reinventar escaping e integra projection/columns |
| XLSX resumido | `poi-ooxml`, SXSSF | Streaming por janela no runtime | fastexcel; CSV continua primário para massa | **Sim, atual** | Versão centralizada; temporários, row window e limites são controlados por properties |
