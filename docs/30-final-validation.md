...
| Build/compilação/formatação/testes | `mvnw.cmd -B -ntp verify -Pstatic-analysis -Dtoolkit.lab=true`: BUILD SUCCESS; **74 testes**, zero falhas/erros e **2 skips**, exclusivamente os benchmarks condicionais já executados separadamente; release 25 sem preview. Os **6 `DockerLabTest`** passaram no mesmo `verify`; Spotless confirmou **105 arquivos Java** limpos |
